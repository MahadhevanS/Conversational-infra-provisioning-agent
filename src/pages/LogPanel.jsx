...
// // Stages shown in the tab bar — cost/show/error are hidden but their text
// // is still included when a tab is active that contains mixed content.
// const VISIBLE_STAGES = new Set(["init", "plan", "apply", "destroy"]);

// const POLL_INTERVAL = 2000;

// // ── Colour-code a single terminal line ──────────────────────────────────────
...
//   // ── Derive visible stages from chunks ──────────────────────────────────────
...
//         // Auto-advance to latest visible stage unless user picked one
//         const latest = [...new Set(data.chunks.map(c => c.stage))]
//           .filter(s => VISIBLE_STAGES.has(s))
//           .pop();

//         setActiveStage(prev => {
//           if (!userPicked || !prev) return latest || prev;
//           return prev;
//         });
//       }
//       setError(null);
...
//   // Combine all chunks for the active stage into a single rendered block.
//   // We show stdout + stderr interleaved in insertion order (chunk order).
//   const activeChunks = activeStage
//     ? chunks.filter(c => c.stage === activeStage)
//     : [];

//   // For copy: include every visible stage
...
//         {/* ── Stage tabs (only visible stages shown) ── */}
...
//               // Count lines across all chunks for this stage
...
//                 const rawText = stripAnsi(chunk.text || "");
...
//           {/* If no stage selected yet but we have chunks, show a hint */}
//           {!loading && jobId && chunks.length > 0 && !activeStage && visibleStages.length === 0 && (
//             <div className="text-zinc-500 text-xs mt-4 text-center">
//               No displayable stages yet...
//             </div>
//           )}
//         </div>
...
const LogPanel = ({ isOpen, onClose, jobId, jobMode, jobStatus, theme }) => {
  ...
  const fetchLogs = useCallback(async (id) => {
    ...
    try {
      const data = await apiFetch(`/logs/${id}`);
      if (data.chunks) {
        let combinedChunks = data.chunks;

        // FIX: If we are in Destroy mode, we often need to fetch the original 
        // setup logs (init/plan) which are likely tied to the parent/related job.
        // If the backend doesn't aggregate them, we check if the job has a parent.
        if (data.parent_job_id && data.job_type === "DESTROY") {
           try {
             const parentData = await apiFetch(`/logs/${data.parent_job_id}`);
             if (parentData.chunks) {
                // Keep only unique stages from parent (init/plan) that aren't in destroy chunks
                const currentStages = new Set(combinedChunks.map(c => c.stage));
                const parentChunks = parentData.chunks.filter(c => !currentStages.has(c.stage));
                combinedChunks = [...parentChunks, ...combinedChunks];
             }
           } catch (e) { console.error("Could not fetch parent logs", e); }
        }
        ...
        // Auto-advance to latest visible stage unless user picked one
        const latest = [...new Set(combinedChunks.map(c => c.stage))]
          .filter(s => VISIBLE_STAGES.has(s))
          .pop();
  ...
  const activeChunks = activeStage ? chunks.filter(c => c.stage === activeStage) : [];
      ...
      `}>
        ...
        {allText.trim().length > 0 && (
          <div className={`px-4 py-3 shrink-0 flex justify-between items-center border-t
            ...
            <span className="text-[10px] text-zinc-500 uppercase tracking-widest">
              {activeStage ? stageMeta(activeStage).label : "All"} · {
                 activeChunks.reduce((n, c) => n + (c.text || "").split("\n").filter(Boolean).length, 0)
              } line{activeChunks.reduce((n, c) => n + (c.text || "").split("\n").filter(Boolean).length, 0) === 1 ? "" : "s"}
            </span>
            <button
              onClick={() => navigator.clipboard.writeText(allText)}
              className="text-[10px] font-bold uppercase tracking-widest text-zinc-500
