...
import ProjectMembersModal from "../pages/ProjectMembersModal";
...
const ConsoleLayout = () => {
  ...
  const triggerCost = async (planJobId) => {
    try {
      ...
      const data = await apiFetch("/cost", {
        method: "POST",
        body: JSON.stringify({ run_id: planJobId, project_id: activeProjectIdRef.current, user_id: currentUserId }),
      });
  ...
  const triggerApply = async (planJobId) => {
    ...
    const body = { project_id: activeProjectIdRef.current, job_id: planJobId, user_id: currentUserId };
